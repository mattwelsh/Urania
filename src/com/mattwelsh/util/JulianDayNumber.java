...
/**
 * This class represents a Julian Day Number, the number assigned to a solar day in the Julian day count starting from
 * noon Greenwich Mean Time, with Julian day number 0 assigned to the day starting at noon on January 1, 4713 BC on
 * the Julian calendar (November 24, 4714 BC, in the Gregorian calendar). Due to the way GregorianCalendar works, this
 * class only works for dates after Jan 1, 1AD.
 *
...
public class JulianDayNumber {
    ...
    private void computeJulianDayNumber() {
        ...
        double D = ((double)gregorianCalendar.get(GregorianCalendar.HOUR_OF_DAY))/24.0 +
        ...
        julianDayNumber = (int)((365.25 * y) + C) + (int)(30.6001 * (m + 1)) +
                gregorianCalendar.get(GregorianCalendar.DAY_OF_MONTH) + D + 1720994.5 + B;
